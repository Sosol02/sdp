...
public class RouteServiceMock implements RouteService {
    ...
    @Override
    public void requestRoutes(@NonNull Coordinate coordinate, @NonNull List<Destination> list,
                              @NonNull RouteOptions routeOptions, @NonNull RoutesResponseListener routesResponseListener) {
        ...
        prompts.add(new Prompt(0.0, Prompt.Placement.AFTER, "Go northeast on 2nd Avenue",
                "Go northeast on 2nd Avenue", 0, 0));
        prompts.add(new Prompt(1.0, Prompt.Placement.BEFORE, "You have arrived at your destination",
                "You have arrived at your destination", 2, 20));

        List<SpeedLimitSpan> speedLimits = new ArrayList<>();
        speedLimits.add(new SpeedLimitSpan(0.0, 1.0, (float) 11.111111,
                SpeedLimit.Type.MAXIMUM, SystemOfMeasurement.UNITED_STATES_CUSTOMARY));

        List<Instruction> instructions = new ArrayList<>();
        instructions.add(new Instruction("Start out going northeast on 2nd Ave", 0.0,
                Maneuver.Type.START, (float) 0.0));
        instructions.add(new Instruction("You have arrived at your destination", 1.0,
                Maneuver.Type.DESTINATION, (float) 0.0));
